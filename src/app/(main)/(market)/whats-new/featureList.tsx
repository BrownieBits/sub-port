...
import { NoFeatures } from './noFeatures';
...
export default function FeaturesListPage() {
  ...
  const [lastFeature, setLastFeature] = React.useState<
    DocumentData | undefined
  >(undefined);
  const { ref, inView } = useInView();

  async function GetPosts() {
    const postsRef: CollectionReference = collection(db, 'features');
    ...
    if (lastFeature !== undefined) {
      postsQuery = query(postsQuery, startAfter(lastFeature));
    }
    ...
    if (poststData.size === 30) {
      setLastFeature(poststData.docs[poststData.size - 1]);
    } else {
      setLastFeature(undefined);
    }
    const newPosts: Post[] = [];
    if (poststData.empty) {
      if (posts === null) {
