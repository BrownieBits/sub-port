...
export default function NewPostForm() {
  ...
  async function onSubmit() {
    try {
      setDisabled(true);
      let slug = form.getValues('name');
      slug = slug.toLowerCase().trim();
      slug = slug.replace(/[^a-z0-9 ]/g, ''); // Remove special characters
      slug = slug.replace(/\s+/g, '-');
      const docRef: DocumentReference = doc(db, 'blog', slug);
      const postDoc = await getDoc(docRef);
      if (postDoc.exists()) {
        toast.error('Post Already Exists', {
          description:
            'A post with the same name already exists. Please choose a different name and try again.',
        });
      } else {
        const banner = await uploadImages(slug);
        await setDoc(docRef, {
    ...
    <section className="relative">
      <section className="mx-auto w-full max-w-[1754px]">
        <section className="flex w-full items-center justify-between gap-4 p-4">
          <h1 className="line-clamp-1">New Post</h1>
