...
export const LatestBlog = (props: {}) => {
    ...
    return (
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          <Skeleton className="bg-layer-two aspect-video w-full rounded" />
          <Skeleton className="bg-layer-two h-5 w-[150px]" />
          <section className="flex w-full flex-col gap-2">
            <Skeleton className="bg-layer-two h-5 w-full" />
            <Skeleton className="bg-layer-two h-5 w-full" />
  ...
  return (
    <section className="bg-layer-one flex w-full flex-col items-start justify-start gap-8 rounded border p-4">
      <h3>SubPort News</h3>
      {blog.fields.banner && (
        <Image
